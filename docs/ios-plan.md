...
1. **iPad 分屏常驻**：Split View / Slide Over 里可见的 App 处于前台活跃态，Paster 放在旁边就能像 Mac 一样轮询 `changeCount` 抓取，无需任何黑科技。这是 iPad 版应当主推的用法。
2. **快捷指令自动化**：个人自动化支持「某 App 打开 / 关闭时」触发，接「获取剪贴板 → 保存到 Paster」，iOS 17 起可免确认立即运行。不是实时，但覆盖「用完某 App 顺手存一下」的场景。
3. **回到前台自动读取**：打开 Paster 即保存当前剪贴板，是最基本的兜底。
4. **借道 Mac**：通用剪贴板把手机复制的内容送到附近的 Mac，Mac 版抓到后经 CloudKit 回流手机。
...
- [ ] 采集通道 C：App Intent「保存剪贴板」→ 快捷指令，引导绑定操作按钮 / 背面轻点，并提供「某 App 关闭时自动保存」的自动化模板
- [ ] iPad：`NavigationSplitView` 侧栏 + 网格，卡片可拖放到 Split View / Slide Over 里的其他 App，硬件键盘快捷键（⌘F 搜索、方向键、回车复制、空格预览）
- [ ] iPad 分屏常驻采集：Paster 处于 Split View / Slide Over 可见状态时持续轮询 `changeCount`，实现接近 Mac 的自动抓取；引导页专门介绍这个用法
